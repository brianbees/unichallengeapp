...
function ScoreScreen() {
  ...
  const bottomY = footerH; // safe area handled in styles
      ...
      <View style={styles.scoreRow}>
        ...
        <TouchableOpacity
          onPress={() =>
            setTeamName(
              teamKey,
              prompt(`Rename Team ${teamKey}`, state.teams[teamKey]) ||
                state.teams[teamKey]
            )
          }
          accessibilityRole="button"
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      {/* Middle layer (Score/Events/History) */}
      <View style={[styles.middleLayer, { top: topY, bottom: bottomY }]}>
        {activeTab === 'Score' && (
          <ScrollView contentContainerStyle={styles.teamGrid}>
            {['A', 'B', 'C', 'D'].map((k) => (
              <TeamCard key={k} teamKey={k} />
            ))}
          </ScrollView>
        )}
        {activeTab === 'Events' && (
